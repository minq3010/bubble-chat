...
export default function BubbleWindow() {
  ...
  useEffect(() => {
    ...
    desktop()?.getSettings().then((settings) => {
      if (settings?.theme) setTheme((prev) => (prev === settings.theme ? prev : settings.theme));
      if (
        settings?.bubbleSize &&
        (settings.bubbleSize === "Small" || settings.bubbleSize === "Medium" || settings.bubbleSize === "Large")
      ) {
    ...
    const offIpc = desktop()?.onAppearance((data) => {
      ...
      if (
        data.bubbleSize &&
        (data.bubbleSize === "Small" || data.bubbleSize === "Medium" || data.bubbleSize === "Large")
      ) {
  ...
  return (
    <div
      className={`${isDark ? "dark" : ""} relative flex h-screen w-screen items-center justify-center bg-transparent select-none`}
      onContextMenu={(e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    >
      <div
        ...
        className={`cursor-grab select-none active:cursor-grabbing ${isDraggingVisual ? "cursor-grabbing" : ""}`}
      >
        <FloatingBubble
