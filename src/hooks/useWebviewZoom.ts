...
export function useWebviewZoom(provider?: string): UseWebviewZoomResult {
  const isChatProvider = provider === "messenger" || provider === "zalo"
  ...
  const getEffectiveZoom = useCallback(
    (baseZoom: number) => {
      if (!isChatProvider) return baseZoom
  ...
  const triggerGuestResize = useCallback(() => {
    ...
    if (view) {
      ...
      if (isChatProvider && typeof view.insertCSS === "function") {
        try {
          void view
            .insertCSS(
              `
  ...
  }, [getEffectiveZoom, isChatProvider])
